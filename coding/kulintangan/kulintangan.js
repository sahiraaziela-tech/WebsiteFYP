  ...
  const start = async () => {
    try {
      const mindarThree = new window.MINDAR.IMAGE.MindARThree({
        ...
        imageTargetSrc: "/WebsiteFYP/assets/targets/markerkulintangan.mind",
      });
      ...
      // --- [BARU] VARIABLE UNTUK ANIMASI ---
      const clock = new THREE.Clock(); // Jam untuk kira masa animasi
      let mixer = null; // Pengawal animasi (Player)
      let hitAction = null; // Aksi khusus (Contoh: "Ketuk")
      ...
      // A. Audio Bunyi Kulintangan (Ketukan Sekali)
      const kulintanganHitSound = new THREE.Audio(listener);
      audioLoader.load("/WebsiteFYP/assets/sound/Kulintangan.mp3", (buffer) => {
        kulintanganHitSound.setBuffer(buffer);
        kulintanganHitSound.setLoop(false); 
        kulintanganHitSound.setVolume(1.0);
      });

      // B. Audio Lagu Kulintangan (Looping)
      const kulintanganMusic = new THREE.Audio(listener);
      audioLoader.load("/WebsiteFYP/assets/sound/LaguKulintangan.mp3", (buffer) => { 
        kulintanganMusic.setBuffer(buffer);
        kulintanganMusic.setLoop(true); 
        kulintanganMusic.setVolume(0.5);
      });
      ...
      const gltf = await new Promise((resolve, reject) => {
        // Pastikan nama fail GLB anda betul (yang ada animasi dari Blender)
        gltfLoader.load("/WebsiteFYP/assets/models/kulintanganbaru.glb", resolve, undefined, reject);
      });
      ...
      // --- [BARU] SETUP ANIMASI DARI BLENDER ---
      const clips = gltf.animations; // Ambil semua animasi dalam fail GLB
      
      if (clips.length > 0) {
          mixer = new THREE.AnimationMixer(model); // Cipta Mixer pada model
          
          // Ambil animasi pertama (atau cari guna nama: clips.find(c => c.name === 'Ketuk'))
          hitAction = mixer.clipAction(clips[0]); 
          
          // Setting: Main sekali sahaja, jangan ulang-ulang
          hitAction.setLoop(THREE.LoopOnce); 
          hitAction.clampWhenFinished = true; // Berhenti di frame akhir (optional)
          
          console.log("Animasi ditemui:", clips[0].name);
      } else {
          console.log("Tiada animasi ditemui dalam fail GLB ini.");
      }

      model.userData = { type: "kulintanganModel" }; 
      anchor.group.add(model);


      // --- B. Butang Speaker 3D (Coklat) ---
      function createMusicButton() {
          ...
          buttonMesh.position.set(0.75, -0.2, 0); 
      ...
      // ================= 5. INTERACTION (MOUSE MIDDLE BUTTON ROTATE) =================
      ...
      window.addEventListener("pointerup", (e) => {
        ...
        if (hits.length > 0) {
            ...
            // Cari parent sampai jumpa root yang ada userData.type
            ...
            if (type === "musicBtn") {
                if (kulintanganMusic.isPlaying) {
                    kulintanganMusic.pause();
                    musicBtn.material.color.setHex(0xaaaaaa); 
                } else {
                    if (kulintanganHitSound.isPlaying) kulintanganHitSound.stop();
                    kulintanganMusic.play();
                    musicBtn.material.color.setHex(0xffffff); 
                }
            }

            // --- [BARU] B. KLIK MODEL KULINTANGAN (ANIMASI + KETUK) ---
            else if (type === "kulintanganModel") {
                
                // 1. Stop lagu background kalau tengah main
                if (kulintanganMusic.isPlaying) {
                    kulintanganMusic.pause();
                    musicBtn.material.color.setHex(0xaaaaaa); 
                }

                // 2. Mainkan Animasi Blender (Jika ada)
                if (hitAction) {
                    hitAction.stop(); // Reset animasi ke frame 0
                    hitAction.play(); // Mainkan
                    
                    // 3. Mainkan bunyi ikut masa (Timing)
                    // Kita letak delay sikit supaya bunyi keluar bila pemukul KENA gong
                    // Ubah nombor '200' ikut kesesuaian animasi anda
                    setTimeout(() => {
                         if (kulintanganHitSound.isPlaying) kulintanganHitSound.stop(); 
                         kulintanganHitSound.play();
                    }, 200); 

                } else {
                    // Fallback: Jika tiada animasi blender, guna gegar biasa
                    if (kulintanganHitSound.isPlaying) kulintanganHitSound.stop(); 
                    kulintanganHitSound.play();
      ...
      // ================= 7. TARGET EVENTS =================
      anchor.onTargetLost = () => {
        if (kulintanganMusic.isPlaying) kulintanganMusic.pause();
        if (kulintanganHitSound.isPlaying) kulintanganHitSound.stop();
      };
      ...
      renderer.setAnimationLoop(() => {
        ...
        // --- [BARU] UPDATE ANIMASI SETIAP FRAME ---
        const delta = clock.getDelta(); // Kira masa yang berlalu
        if (mixer) {
            mixer.update(delta); // Gerakkan animasi blender
        }

        // Animasi butang muzik
        if(musicBtn) {
